...
class ReplayBuffer:
    def __init__(self,buffer_capacity, batch_size,n_agents,states,actions):
        ...
        self.list_actor_n_actions=[actions[index].shape[0] for index in range(len(self.n_agents))]
        #self.list_actor_n_actions=[rewards[index].shape[0] for index in range(len(self.n_agents))]
        ...
        self.dones = np.zeros((self.buffer_capacity, len(self.n_agents)), dtype=bool)

        self.list_actors_states = []
        self.list_actors_next_states = []
        self.list_actors_actions = []
        self.list_actors_rewards = []
        ...
        for n in range(len(self.n_agents)):
            ...
            #self.list_actors_rewards.append(np.zeros((self.buffer_capacity, self.list_actor_n_rewards[n])))
    ...
    def add_record(self, actor_states, actor_next_states, actions, state, next_state, reward,done):
        ...
        for agent_index in range(len(self.n_agents)):
            ...
            #self.list_actors_rewards[agent_index][index] = rewards[agent_index]
    ...
    def get_minibatch(self):
        ...
        actors_state = [self.list_actors_states[index][batch_index] for index in range(len(self.n_agents))]
        actors_next_state = [self.list_actors_next_states[index][batch_index] for index in range(len(self.n_agents))]
        actors_action = [self.list_actors_actions[index][batch_index] for index in range(len(self.n_agents))]
        #actors_rewards = [self.list_actors_rewards[index][batch_index] for index in range(len(self.n_agents))]
